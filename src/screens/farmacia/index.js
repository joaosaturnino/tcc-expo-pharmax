...
import { 
  ...
  ScrollView,
  ActivityIndicator,
  Linking // Importante para abrir telefone/email
} from 'react-native';
...
export default function Farmacia() {
  ...
  // Estado para guardar detalhes (Endereço, email, telefone...)
  const [farmInfo, setFarmInfo] = useState(null);
  const [medicamentos, setMedicamentos] = useState([]);
  const [loading, setLoading] = useState(true);
  const [isFavorito, setIsFavorito] = useState(false);

  const favoritosCount = 1250; // Exemplo estático ou vindo do banco
  ...
  // --- Busca de Dados (Info da Farmácia + Medicamentos) ---
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        if (resInfo.data.sucesso && resInfo.data.dados) {
           // Verifica se veio um array (comum em alguns endpoints) ou objeto
  ...
  return (
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {/* --- NOVA SEÇÃO: INFORMAÇÕES DA FARMÁCIA --- */}
        {farmInfo && (
                ...
                <View style={styles.infoRow}>
                    ...
                    <View style={styles.infoTexts}>
                        ...
                        <Text style={styles.infoValue}>
                            {farmInfo.farm_endereco ? farmInfo.farm_endereco : 'Endereço não informado'}
